...
if(copyFileToLocal($fileid)===TRUE){
	...
	$logfile = fopen("task_logs.txt","w");
	fwrite($logfile, $runresult);
	fclose($logfile);
	#verification task if no score file generated
	#incomprehensive error checking...need to improve	
	if(!file_exists($scorefile)){
		echo "Verification task failed...\nThe developer is notified with this problem. Sorry for the inconvenience!\n";
		error_log("Attendance System:\nVerification: Verification run failed.", 1, "dingz@andrew.cmu.edu");
	}
	#read scores file 
	#$command = "cat " . $scorefile ." | sort -n -r -k 4 | head -n 1 sorted.txt | cut -d' ' -f2";
	$scorelines = file($scorefile);
	usort($scorelines, "cmp");
	$firstline = explode(" ", $scorelines[0]);
	$firstmatch = $firstline[1];
	$secondline = explode(" ", $scorelines[1]);
	$secondmatch = $secondline[1];
	$thirdline = explode(" ", $scorelines[2]);
	$thirdmatch = $thirdline[1];	
	#echo $bestmatch."\n";
	echo "RESULTS:".$firstmatch."/".$secondmatch."/".$thirdmatch."\n";
	#unlink($scorefile);
	#unlink("sorted.txt");
	removeFromModelsList();
	chdir("/var/www/");
